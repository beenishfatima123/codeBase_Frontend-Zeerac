...
import { reportAgency, resetAgency } from "../../../redux/slices/partnersSlice";
...
/* AgencyReport is a Modal which appears when "Looks fake? Report it." button
on the Agency page is clicked, it takes open bool, setOpen method. 
open is used to display Modal, setOpen will change open's value. */

const ReportAgency = ({ open, setOpen }) => {
  ...
  const { reportAgencyDataApiInfo, partnerDetails } = useSelector(
  ...
  useEffect(() => {
    if (reportAgencyDataApiInfo?.response) {
      toast.success(reportAgencyDataApiInfo?.response?.message, {
  ...
  }, [reportAgencyDataApiInfo?.response]);
  useEffect(() => {
    if (reportAgencyDataApiInfo?.error) {
      toast.error(reportAgencyDataApiInfo?.response?.message, {
        position: toast.POSITION.TOP_CENTER,
        hideProgressBar: true,
      });
      dispatch(resetAgency());
    ...
    <Modal open={open}>
      <Box sx={style}>
        ...
        <div className={classes.container}>
          {reportAgencyDataApiInfo?.loading ? (
            <ComponentLoader />
          ) : (
            <Grid container justifyContent="space-around" spacing={2}>
              ...
              <Grid item xs={12} sm={12} md={11.5} lg={11.5} xl={11.5}>
                <button
                  ...
                  onClick={async () => {
                    const isValid = await isAgencyFormValid(user);
