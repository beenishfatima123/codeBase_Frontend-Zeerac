...
const ListingReport = ({ open, setOpen, property }) => {
  ...
  const { reportPropertyDataApiInfo } = useSelector(
  ...
  const isListingFormValid = async (data) => {
  ...
  const handleSubmit = () => {
    ...
    formData.append("listing", user?.listing);
  ...
  useEffect(() => {
    if (reportPropertyDataApiInfo?.response) {
      toast.success(reportPropertyDataApiInfo?.response?.message, {
        position: toast.POSITION.TOP_CENTER,
        hideProgressBar: true,
      });
      dispatch(resetReportProperty());
      setUser({
        name: "",
        email: "",
        message: "",
  ...
  }, [reportPropertyDataApiInfo?.response]);
  useEffect(() => {
    if (reportPropertyDataApiInfo?.error) {
      toast.error(reportPropertyDataApiInfo?.response?.message, {
        position: toast.POSITION.TOP_CENTER,
        hideProgressBar: true,
      });
      dispatch(resetReportProperty());
    ...
    // eslint-disable-next-line
  }, [reportPropertyDataApiInfo?.error]);
    ...
    <Modal open={open}>
      <Box sx={style}>
        ...
        <div className={classes.container}>
          {reportPropertyDataApiInfo?.loading ? (
            <ComponentLoader />
          ) : (
            <Grid container justifyContent="space-around" spacing={2}>
              ...
              <Grid item xs={12} sm={12} md={11.5} lg={11.5} xl={11.5}>
                <button
                  ...
                  onClick={async () => {
                    const isValid = await isListingFormValid(user);
